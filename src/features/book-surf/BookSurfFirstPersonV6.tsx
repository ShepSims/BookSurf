...
export default function BookSurfGameCanvas(){
  ...
  useEffect(()=>{
    ...
    const facePoint=(vw:number,vh:number,u:number,v:number,g:Runtime)=>{const crestX=vw*(.045+g.pocket*.035),lipY=vh*(.27+(1-g.face)*.045),troughY=vh*.95,x=crestX+(vw*1.08-crestX)*u,top=lipY+Math.pow(u,.72)*vh*.205+Math.pow(u,2.25)*vh*.17,bottom=troughY-Math.sin(clamp(u,0,1)*Math.PI)*vh*.055;return{x,y:top+(bottom-top)*v,top,bottom,crestX,lipY,troughY}};
    ...
    const drawMain=(vw:number,vh:number,g:Runtime)=>{
      ...
      const crestX=vw*(.045+g.pocket*.035),lipY=vh*(.27+(1-g.face)*.045),troughY=vh*.95,shoulderY=vh*(.48+(1-g.face)*.1);ctx.beginPath();ctx.moveTo(-vw*.2,troughY);ctx.bezierCurveTo(-vw*.08,vh*.58,crestX-30,lipY+28,crestX,lipY);ctx.bezierCurveTo(vw*.18,lipY+20,vw*.40,shoulderY-34,vw*.63,shoulderY);ctx.bezierCurveTo(vw*.83,shoulderY+55,vw*1.03,troughY-110,vw*1.2,troughY-28);ctx.lineTo(vw*1.2,vh*1.2);ctx.lineTo(-vw*.2,vh*1.2);ctx.closePath();const wave=ctx.createLinearGradient(crestX,lipY,vw*.76,troughY);wave.addColorStop(0,"#063f5a");wave.addColorStop(.18,"#07748f");wave.addColorStop(.43,"#0a8ca4");wave.addColorStop(.7,"#087389");wave.addColorStop(1,"#043e52");ctx.fillStyle=wave;ctx.fill();
