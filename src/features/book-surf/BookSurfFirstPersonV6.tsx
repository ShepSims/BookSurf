...
export default function BookSurfGameCanvas(){
  ...
  useEffect(()=>{
    ...
    type WaveShape={proximity:number;crestX:number;lipY:number;troughY:number;shoulderX:number;shoulderY:number;farY:number;wallRise:number;crestTangent:number;shoulderTension:number;shoulderLift:number};
    const rawWaveShape=(vw:number,vh:number,g:Runtime):WaveShape=>{const proximity=1-smooth(clamp((g.pocket-.04)/.88,0,1)),crestX=vw*(-.12+proximity*.31),lipY=vh*(.15+g.face*.16),troughY=vh*.98,shoulderX=vw*(.48+(1-proximity)*.1),shoulderY=vh*(.43+(1-g.face)*.12+proximity*.035),farY=vh*(.53+(1-g.face)*.08),wallRise=vh*(.20+proximity*.18),crestTangent=vw*(.07+(1-proximity)*.1),shoulderTension=vw*(.08+proximity*.14),shoulderLift=vh*(.025+proximity*.085);return{proximity,crestX,lipY,troughY,shoulderX,shoulderY,farY,wallRise,crestTangent,shoulderTension,shoulderLift}};
    const surfaceYAt=(vw:number,x:number,s:WaveShape)=>{const left=-vw*.18,right=vw*1.15;if(x<=s.crestX){const t=clamp((x-left)/(s.crestX-left),0,1);return cubic(s.troughY,s.lipY+s.wallRise,s.lipY+Math.max(12,s.wallRise*.12),s.lipY,t)}if(x<=s.shoulderX){const t=clamp((x-s.crestX)/(s.shoulderX-s.crestX),0,1);return cubic(s.lipY,s.lipY-s.shoulderLift*.42,s.shoulderY-s.shoulderLift,s.shoulderY,t)}const t=clamp((x-s.shoulderX)/(right-s.shoulderX),0,1);return cubic(s.shoulderY,s.shoulderY+18,s.farY-20,s.farY,t)};
    const waveShape=(vw:number,vh:number,g:Runtime):WaveShape=>{const raw=rawWaveShape(vw,vh,g),playerSurface=surfaceYAt(vw,vw*.52,raw),cameraOffset=vh*.875-playerSurface;return{...raw,lipY:raw.lipY+cameraOffset,troughY:raw.troughY+cameraOffset,shoulderY:raw.shoulderY+cameraOffset,farY:raw.farY+cameraOffset}};
    // Particles and the filled face sample this exact same surface, so they cannot float in sky.
    const facePoint=(vw:number,vh:number,u:number,v:number,g:Runtime)=>{const s=waveShape(vw,vh,g),x=vw*.52+(u-clamp(g.pocket,0,1))*vw*.92,top=surfaceYAt(vw,x,s),bottom=vh*(1.04-Math.sin(clamp(u,0,1)*Math.PI)*.04);return{x,y:top+(bottom-top)*clamp(v,0,1),top,bottom,crestX:s.crestX,lipY:s.lipY,troughY:s.troughY}};
    ...
    const drawMain=(vw:number,vh:number,g:Runtime)=>{
      ...
      const shape=waveShape(vw,vh,g),{proximity,crestX,lipY,troughY,shoulderX,shoulderY,farY,wallRise,crestTangent,shoulderTension,shoulderLift}=shape;
      // Keep the visible wave bounded and curved. Player position shifts the pocket left/right,
      // while face height changes the apparent crest and shoulder height without projecting a
      // world-space polygon through the camera (which produced the giant diagonal wedges).
      ctx.beginPath();ctx.moveTo(-vw*.18,troughY);ctx.bezierCurveTo(-vw*.08,lipY+wallRise,crestX-vw*(.018+proximity*.035),lipY+vh*(.025+proximity*.025),crestX,lipY);ctx.bezierCurveTo(crestX+crestTangent,lipY-vh*(.012+proximity*.035),shoulderX-shoulderTension,shoulderY-shoulderLift,shoulderX,shoulderY);ctx.bezierCurveTo(vw*.72,shoulderY+vh*(.025+(1-proximity)*.025),vw*.96,farY-vh*.04,vw*1.15,farY);ctx.lineTo(vw*1.15,vh*1.22);ctx.lineTo(-vw*.18,vh*1.22);ctx.closePath();const wave=ctx.createLinearGradient(crestX,lipY,vw*.72,vh);wave.addColorStop(0,"#063f5a");wave.addColorStop(.18,"#07748f");wave.addColorStop(.43,"#0a8ca4");wave.addColorStop(.7,"#087389");wave.addColorStop(1,"#043e52");ctx.fillStyle=wave;ctx.fill();
      ...
      const pitch=clamp(proximity*(.55+g.face*.45),.03,1),reach=vw*(.07+pitch*.17);ctx.beginPath();ctx.moveTo(crestX-12,lipY+8);ctx.bezierCurveTo(crestX+8,lipY-24-pitch*34,crestX+reach*.62,lipY-18-pitch*26,crestX+reach,lipY+44+pitch*38);ctx.bezierCurveTo(crestX+reach*.68,lipY+36,crestX+reach*.31,lipY+26,crestX-12,lipY+8);const lip=ctx.createLinearGradient(crestX,lipY,crestX+reach,lipY+90);lip.addColorStop(0,"rgba(245,255,255,.98)");lip.addColorStop(.24,"rgba(170,230,236,.88)");lip.addColorStop(.65,"rgba(32,137,159,.55)");lip.addColorStop(1,"rgba(4,72,94,.06)");ctx.fillStyle=lip;ctx.fill();ctx.fillStyle="rgba(0,33,51,.32)";ctx.beginPath();ctx.moveTo(crestX+6,lipY+18);ctx.bezierCurveTo(crestX+reach*.28,lipY+46,crestX+reach*.4,lipY+100,crestX+reach*.56,lipY+145);ctx.bezierCurveTo(crestX+reach*.2,lipY+132,crestX-7,lipY+78,crestX-12,lipY+36);ctx.closePath();ctx.fill();
      ctx.save();ctx.beginPath();ctx.moveTo(-vw*.18,troughY);ctx.bezierCurveTo(-vw*.08,lipY+wallRise,crestX-vw*(.018+proximity*.035),lipY+vh*(.025+proximity*.025),crestX,lipY);ctx.bezierCurveTo(crestX+crestTangent,lipY-vh*(.012+proximity*.035),shoulderX-shoulderTension,shoulderY-shoulderLift,shoulderX,shoulderY);ctx.bezierCurveTo(vw*.72,shoulderY+vh*(.025+(1-proximity)*.025),vw*.96,farY-vh*.04,vw*1.15,farY);ctx.lineTo(vw*1.15,vh*1.22);ctx.lineTo(-vw*.18,vh*1.22);ctx.closePath();ctx.clip();
      for(const p of g.particles){if(p.kind!=="face")continue;const pt=facePoint(vw,vh,p.u,p.v,g),alpha=.22+.46*(1-p.v)*(.55+.45*rand(p.seed));ctx.globalAlpha=alpha;ctx.fillStyle=p.v<.32?"#c8eef2":"#79ced7";ctx.beginPath();ctx.arc(pt.x,pt.y,p.size,0,TAU);ctx.fill()}ctx.globalAlpha=1;for(const p of g.particles){if(p.kind!=="foam"&&p.kind!=="mist")continue;const a=clamp(p.life/p.maxLife,0,1),pt=facePoint(vw,vh,p.u,clamp(p.v,0,1),g);ctx.globalAlpha=a*(p.kind==="mist"?.58:.88);ctx.fillStyle=p.kind==="mist"?"#f2ffff":"#dbf3f2";ctx.beginPath();ctx.arc(pt.x-(p.kind==="mist"?(1-a)*22:0),pt.y-(p.kind==="mist"?(1-a)*18:0),p.size,0,TAU);ctx.fill()}ctx.globalAlpha=1;ctx.restore();ctx.restore();
