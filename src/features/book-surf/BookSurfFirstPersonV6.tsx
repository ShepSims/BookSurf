...
export default function BookSurfGameCanvas(){
  ...
  useEffect(()=>{
    ...
    const drawMain=(vw:number,vh:number,g:Runtime)=>{
      ...
      const proximity=1-smooth(clamp((g.pocket-.04)/.88,0,1));
      ...
      const crestX=vw*(-.12+proximity*.31),lipY=vh*(.15+g.face*.16),troughY=vh*.98;
      const shoulderX=vw*(.48+(1-proximity)*.1),shoulderY=vh*(.43+(1-g.face)*.12+proximity*.035),farY=vh*(.53+(1-g.face)*.08);
      const wallRise=vh*(.20+proximity*.18),crestTangent=vw*(.07+(1-proximity)*.1),shoulderTension=vw*(.08+proximity*.14),shoulderLift=vh*(.025+proximity*.085);
