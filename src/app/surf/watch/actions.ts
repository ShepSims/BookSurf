...
export async function createWatch(formData: FormData) {
  ...
  if (!parsed.success || parsed.data.maxTripNights < parsed.data.minTripNights) redirect("/surf/watch?error=invalid");
  const supabase = await createSupabaseServerClient();
  if (!supabase) redirect("/surf/watch?error=supabase");
  const { data: userData } = await supabase.auth.getUser();
  if (!userData.user) redirect("/account?next=/surf/watch");
  const v = parsed.data;
  const { error } = await supabase.from("surf_watches").insert({
  ...
  if (error) redirect("/surf/watch?error=save");
  redirect("/surf/opportunities?watch=created");
}
