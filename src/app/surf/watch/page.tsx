...
export default async function WatchPage({ searchParams }: { searchParams: Promise<{error?: string}> }) {
  ...
  return <main className="container" style={{padding:"64px 0 100px", maxWidth:760}}><div className="eyebrow">Create surf watch</div><h1 style={{fontSize:52, letterSpacing:"-.055em", margin:"12px 0 8px"}}>What’s a trip worth taking?</h1><p style={{color:"var(--muted)", marginBottom:30}}>The MVP uses the live marine/weather horizon (up to 16 days) and mock travel providers unless commercial credentials are configured.</p>{error && <p className="panel" style={{padding:14, borderColor:"#d79b91"}}>Could not save the watch ({error}). Sign in first and check Supabase configuration.</p>}<form action={createWatch} className="panel" style={{padding:28, display:"grid", gap:20}}>
    ...
    <button className="button" type="submit">Start watching</button>
  </form></main>;
}
